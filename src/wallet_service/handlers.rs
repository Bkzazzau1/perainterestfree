...
// --- UPDATED IMPORT ---
use crate::wallet_service::models::{HistoryQuery, Transaction, WalletSummary};
// ----------------------
use serde_json::json;
...
async fn provision_wallets(pool: &sqlx::PgPool, user_id: Uuid) -> Result<(), AppError> {
    // --- ADDED ---
    debug!(user_id = %user_id, "Checking/provisioning default wallets");
    // -------------
...
pub async fn get_wallet_summary(
    ...
    let wallet_summaries: Vec<WalletSummary> = wallets
        .into_iter()
        .map(|w| {
    ...
    // --- ADDED ---
    info!(user_id = %user_id, "Fetched wallet summary");
    // -------------
...
pub async fn get_transactions(
    ...
    let search_term = query.q.as_ref().map(|s| format!("%{}%", s.to_lowercase()));
    ...
    // --- ADDED ---
    info!(user_id = %user_id, ?query, "Fetched transactions");
    // -------------
