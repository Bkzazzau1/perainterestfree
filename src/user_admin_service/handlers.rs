...
use crate::user_admin_service::models::{
    UserSummary, UserFullProfile, KycUpdatePayload
};
...
pub async fn list_users(
    ...
    let search = query.q.map_or("".to_string(), |q| format!("%{}%", q.to_lowercase()));
...
pub async fn get_user_detail(
    ...
    let (bvn, nin) = if let Some(profile) = profile_info {
        let bvn = profile.bvn_encrypted
            .and_then(|b| state.crypto_service.decrypt(&b).ok());
        let nin = profile.nin_encrypted
    ...
    let full_profile = UserFullProfile {
        ...
        country: bvn.2.as_ref().and_then(|p| p.country.clone()),
        surname: bvn.2.as_ref().and_then(|p| p.surname.clone()),
        first_name: bvn.2.as_ref().and_then(|p| p.first_name.clone()),
        dob: bvn.2.as_ref().and_then(|p| p.dob),
        address: bvn.2.as_ref().and_then(|p| p.address.clone()),
        bvn: bvn.0,
        nin: bvn.1,
        id_type: bvn.2.as_ref().and_then(|p| p.id_type.clone()),
        occupation: bvn.2.as_ref().and_then(|p| p.occupation.clone()),
    };
