...
use crate::admin_management_service::models::{
    CreateRolePayload, AssignRolePayload
};
...
pub async fn list_roles(
    ...
    let roles: Vec<_> = records.into_iter().map(|r| {
        json!({
            "id": r.role_id,
...
pub async fn list_permissions(
    ...
    let result: Vec<_> = perms.into_iter().map(|p| {
        json!({ "key": p.permission_key, "description": p.description })
    }).collect();
