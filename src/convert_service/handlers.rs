...
pub async fn execute_conversion(
    ...
    if payload.from_currency == payload.to_currency {
        return Err(AppError::ProviderError("Cannot convert to same currency".to_string()));
    }
    ...
    let api_key = settings.get("brails_api_key")
        .ok_or(AppError::ProviderError("Brails API key not set".to_string()))?;
    let markup: f64 = settings.get("fx_rate_usd_ngn_markup")
        .unwrap_or(&"0.0".to_string())
        .parse().unwrap_or(0.0);

    let from_key = currency_to_wallet_key(&payload.from_currency);
    let to_key = currency_to_wallet_key(&payload.to_currency);
    
    // --- 4. Get Live Rate from Brails (not mock) ---
    let live_rate = if from_key == to_key {
        1.0
    } else {
        state.brails_client.get_exchange_rate(api_key, from_key, to_key)
            .await
    ...
    // 6. Calculate amounts
    let from_amount_minor = payload.amount_minor;
    ...
    .await.map_err(AppError::DatabaseError)?
    .ok_or(AppError::ProviderError(format!("{} wallet not found", from_key)))?;
    ...
    .fetch_optional(&mut *tx)
    .await.map_err(AppError::DatabaseError)?
    .ok_or(AppError::ProviderError(format!("{} wallet not found", to_key)))?;

    // 11. Debit FROM wallet
    sqlx::query!(
        "UPDATE wallets SET balance_minor = balance_minor - $1 WHERE id = $2",
        payload.amount_minor, from_wallet.id
    )
    .execute(&mut *tx)
    .await.map_err(AppError::DatabaseError)?;
    ...
    sqlx::query!(
        ...
        from_key,
        format!("Convert to {}", payload.to_currency), // title
    ...
    sqlx::query!(
        ...
        to_amount_minor,
        to_key,
        format!("Convert from {}", payload.from_currency), // title
    ...
    let response = ConvertResponse {
        ...
        from_amount_minor: payload.amount_minor,
